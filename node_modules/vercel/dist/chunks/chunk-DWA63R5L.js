...
function prettyPrintLogline({
  ...
  const date = (0, import_format.default)(timestampInMs, dateTimeFormat);
...
function printInspectUrl(inspectorUrl) {
  ...
  printAlignedLabel("Inspect", import_chalk2.default.cyan(inspectorUrl));
}
async function processDeployment({
  ...
  let rollingRelease = linkedProject?.rollingRelease;
    ...
    for await (const event of (0, import_client.createDeployment)(clientOptions, requestBody)) {
      ...
      if (event.type === "alias-assigned") {
        ...
        if (event.payload.target === "production" && event.payload.alias && event.payload.alias.length > 0) {
          ...
          printAlignedLabel("Aliased", import_chalk2.default.cyan(prodUrl), { gutter: "\u25B2" });
        }
...
// src/util/deploy/print-deployment-status.ts
function bareDuration(stamp) {
  return (0, import_strip_ansi2.default)(stamp).replace(/^\[|\]$/g, "");
}
async function printDeploymentStatus(client, {
  readyState,
  aliasError,
  indications,
  aliasWarning,
  ...
  if (!isStillBuilding && readyState === "READY") {
    ...
    output_manager_default.print(
      `
${import_chalk6.default.green("\u2713")} ${import_chalk6.default.bold("Ready")} ${import_chalk6.default.dim(`in ${duration}`)}
`
    );
  }
  if (aliasError) {
